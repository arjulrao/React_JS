...
const AddFriend = (/* {image, name, profession} */ { values, handleClick, cardindex }) => {
    ...
    return (
        <div className='w-52 h-fit bg-zinc-200 rounded-md overflow-hidden'>
            ...
            <div className='w-full p-3 flex flex-col items-center gap-[10px]'>
                ...
                <button onClick={() => handleClick(cardindex)} className='w-fit py-3 px-5 bg-pink-700 rounded-xl text-xs font-semibold text-[#f0f0f0] cursor-pointer'>{values.friends === true ? "Friends" : "Add Friends"}</button>
            </div>     
        </div>
